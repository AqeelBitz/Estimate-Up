...
const Navbar = ({ isAuthenticated, handleLogout }) => {
    ...
    const [showMessage, setShowMessage] = useState({ type: '', message: '', subMessage: '' }); // Message state
    ...
    const handleLogoutClick = async () => {
        ...
        try {
            ...
            setShowMessage({ type: 'success', message: 'Logout Successful', subMessage: 'Redirecting...' });
    
            setTimeout(() => {
                window.location.href = '/'; // Redirect after message
            }, 2000);
        } catch (error) {
            setShowMessage({ type: 'error', message: 'Logout Failed', subMessage: 'Please try again.' });
        } finally {
            setIsLoading(false);
            setTimeout(() => setShowMessage({ type: '', message: '', subMessage: '' }), 3000);
        }
    };
                ...
                <div className="container-fluid">
                    ...
                    <div className={`collapse navbar-collapse ${isNavOpen ? 'show' : ''}`}>
                        ...
                        <ul className="navbar-nav right-nav-items">
                            {isAuthenticated ? (
                                <li className="nav-item">
                                    <button className="mysubmit" type="button" onClick={handleLogoutClick} disabled={isLoading}>
                ...
                <div className="message-card-overlay">
                    <div className={`message-card ${showMessage.type}`}>
                        ...
                        <div className="message-text-container">
                            <p className="message-text">{showMessage.message}</p>
                            <p className="sub-text">{showMessage.subMessage}</p>
                        </div>
                    </div>
                </div>
            )}
